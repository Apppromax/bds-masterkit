...
export default function SalesStrategy() {
    ...
    const handleGenerate = async () => {
        ...
        const cost = 10;
        const hasCredits = await checkAndDeductCredits(cost, `Chiến thuật: ${activeCard.label}`);
        if (!hasCredits) {
        ...
        try {
            ...
            if (res) {
                setResult(res);
                toast.success('Quân sư đã phân tích xong!');
                ...
                <div className="xl:col-span-5 flex flex-col md:h-full md:overflow-y-auto no-scrollbar pb-6">
                    <div className="bg-[#1a2332] p-5 rounded-[2rem] border border-white/5 shadow-2xl space-y-4 relative overflow-hidden">
                        ...
                        >
                            ...
                            {isGenerating ? 'QUÂN SƯ ĐANG PHÂN TÍCH...' : `NHẬN CHIẾN THUẬT (${totalSelectedTags} triệu chứng)`}
                        </button>
                    </div>
                </div>
